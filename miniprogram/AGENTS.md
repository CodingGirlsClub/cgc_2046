...
4. **e2e 走 mock transport**（`CGC_E2E_MOCK=true` 构建；`pnpm e2e` 与 shell 脚本会自己构建）。唯一例外是 `flashback-guest.e2e.py`，它打真实后端。样例与流转逻辑在 `src/api/mockTransport.ts`：加字段/加页面要同步改它，否则 `parseOrderKind` 这类 fail-closed 解析会直接把页面打成错误态，e2e 红得莫名其妙。
