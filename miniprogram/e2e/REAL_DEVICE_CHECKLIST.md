...
- [ ] **闪念间分享三件（U9/定稿③）**：我的闪念间 → 「分享 · 把这一刻做成卡片」→（a)「转发给好友」卡片标题=「我找到了 N 年前的自己 · 闪念间」（未寄出/无时间戳落「当年的自己」）；(b)「朋友圈」入口提示右上角菜单；(c)「保存卡片」走 canvas 3:4 摘要卡落相册（金句/今天行/年份脚注；雾面句不出现在卡上），首次触发相册权限弹窗、拒绝后引导设置页。（**真机验证批次**：canvas→相册权限闭环与转发卡片标题平台渲染不可由 mock 证明）
