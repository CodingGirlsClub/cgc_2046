defmodule Cgc2046.Workers.ResearchProgressWorker do
  @moduledoc """
  教研 run 完成判定扫描(切片 H U5, #180;Q7 discharge,KTD5)。

  Oban cron 每 5 分钟一拍(与 LearningProgressWorker 同节奏,`config.exs`),
  扫 `type=research` 且非终态(pending/running/waiting)的 run:

  - run 锚定课程的 ResearchOutput(kind=:issues)已存在 → 调既有
    `:complete` action 置 `succeeded`(内容提交即完成——`save_course_content`
    落库是唯一产出确认信号);
  - 无内容不动(waiting 教研产出);
  - 已终态不动(查询限定非终态,幂等)。

  key 解析:run `input_snapshot["key"]` 形如 `course_<id>`(research
  instantiator 写入约定;event 型 key 不消费——Event 的教研产出后置)。
  ...
  alias Cgc2046.Workflows.{ResearchOutput, WorkflowRun}
  ...
  def perform(%Oban.Job{}) do
    ...
    if completed > 0 do
      Logger.info("research progress sweep: #{completed} run(s) completed")
    end
  ...
  # 判定 = 课程内容存在(input key course_<id> → ResearchOutput 同 key)
  ...
  defp complete_run(run) do
    case run
      ...
      {:error, reason} ->
        Logger.warning(
          "ResearchProgressWorker complete failed for run #{run.id}: #{inspect(reason)}"
        )
  ...
  # 非终态且定义 type=research 的 run(research_run_reaper 的查询过滤同款;
  # WorkflowRun multitenancy global?(true):无 tenant 全局读)
  defp research_active_runs do
    WorkflowRun
