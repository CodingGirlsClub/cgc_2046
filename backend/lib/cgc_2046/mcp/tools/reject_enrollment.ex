defmodule Cgc2046.Mcp.Tools.RejectEnrollment do
  ...
  def description do
    """
    工作台 Owner/Admin 专用：拒绝一条待审批（pending）的报名。rejection_reason 可选，会随拒绝通知发给
    报名人。报名不是 pending 时直接返回错误。
  ...
  def execute_confirmed(actor, params) do
    ...
    with {:ok, enrollment} <- fetch_enrollment(actor, workspace_id, enrollment_id) do
      case enrollment
        ...
        {:error, err} ->
          {:error, Cgc2046.Mcp.Errors.message(err, "failed to reject enrollment")}
      end
    end
  end
