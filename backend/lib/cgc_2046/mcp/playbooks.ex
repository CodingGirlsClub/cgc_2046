defmodule Cgc2046.Mcp.Playbooks do
  ...
  5. 活动生命周期（create_event 直接写,其余四件走确认流）:
     ...
     - update_event(workspace_id, event_id, ...) 改标题/描述/场地/定价/报名策略/赞助配置等;pricing_enabled 改 false 会批量免缴该活动全部待支付报名(与课程同语义,摘要展示受影响笔数);改 initiative_id 会换挂载并按新规则重新快照,响应 inherited 同款回传(未改挂载时只回 locked 项);
