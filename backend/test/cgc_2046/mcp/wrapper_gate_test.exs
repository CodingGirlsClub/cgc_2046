defmodule Cgc2046.Mcp.WrapperGateTest do
  ...
  - 派生门控集合恰为 37 个豁免工具（精确名单：3 × workspace_id: :optional +
  ...
  # 精确名单（与 server.ex 注册的 81 工具一一对应）
  ...
  describe "派生门控集合 = 37 个豁免工具（精确名单）" do
    ...
    test "注册工具数 = 81 且名单完备（无未收录工具）" do
      meta_map = tool_meta_map()

      assert map_size(meta_map) == 81
