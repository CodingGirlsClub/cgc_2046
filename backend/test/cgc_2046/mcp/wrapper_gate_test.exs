defmodule Cgc2046.Mcp.WrapperGateTest do
  ...
  - member-only 工具不携带豁免 meta（44 个：原 11（含 #366 发现面
    list_workspace_courses）+ 工作台管理面 19 + 教研流程 9 + 主理人 3 + 学习分析 1 +
    #596 挂载前预览 1）
  ...
  # 精确名单（与 server.ex 注册的 82 工具一一对应）
  ...
  describe "派生门控集合 = 38 个豁免工具（精确名单）" do
    ...
    test "注册工具数 = 82 且名单完备（无未收录工具）" do
      meta_map = tool_meta_map()

      assert map_size(meta_map) == 82
