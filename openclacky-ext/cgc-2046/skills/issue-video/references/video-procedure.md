...
toc.json / full.md / images 等教材解析产物，不存视频。**媒体产物统一落在课程工作区
的 `media/` 子目录**（workbench 约定：`{工作区}/media/videos/<章id>/`）：

```
<课程工作区>/media/videos/<章id>/
...
7. **落盘与报告**：先过**交付自查**再落盘——
   ...
   补末场；中帧无角标回第 6 步。自查通过后：成片归位 `media/videos/<章id>/`；课程内容里
