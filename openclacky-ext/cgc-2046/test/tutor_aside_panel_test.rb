...
# 教研侧栏(session.aside,attach cgc-tutor)静态合同断言。
#
# 行为面(mcp_error 事件 → 断连横幅渲染 → 按钮跳回 hub)在 JS harness
# (test/panel_behavior_harness.js 的 tutor_aside_mcp_error 场景);本文件
# 保留 harness 执行不到的源级合同锚。
#
# 运行(需项目 mise 环境):cd openclacky-ext/cgc-2046 && mise exec -- ruby test/tutor_aside_panel_test.rb
...
class TutorAsidePanelTest < Minitest::Test
  ...
  def test_mcp_error_banner_wired
    # plan 021:此前全包仅 hub 面板订阅 mcp_error,tutor 侧栏断连零感知——
    ...
    assert_includes VIEW, "cgta-mcp-banner"
    assert_includes VIEW, "cgta-banner-goto"
  end
end
