...
> Measured revision (2026-08-06): `getComputedStyle` was used to extract `body.fontFeatureSettings` from 10 Linear pages; every page returned `"normal"`, meaning Linear's current rendering does **not enable `cv01`/`ss03`**. Below, the feature is retained as a "design intent / recommendation" rather than a description of Linear's actual behavior. The same measurement also found heading weights of 500, not the 510/590 stated in this document — see the ⚠️ notes at each occurrence.

---

The typographic system reveals Linear's engineering ethos with unusual precision. Inter Variable is the sole typeface, but it is deployed at weight 510 — a value that exists between regular (400) and medium (500), made possible only by variable font technology. This is not an accident or a rounding error. Weight 510 is the signature choice: it creates emphasis that registers subconsciously without ever shouting, a typographic whisper louder than a bold. For subheadings, 590 occupies a similar in-between space — heavier than medium, lighter than semibold — producing a hierarchy that feels calibrated rather than stepped. ⚠️ Measured: across 10 Linear pages, headings actually render at weight 500, not 510/590 — the latter is this spec's recommended design intent and may be implemented as such under a variable font. OpenType features `cv01` and `ss03` ⚠️ Measured: Linear's current rendering has `fontFeatureSettings = "normal"` — they are not enabled; the original description presenting them as "globally active" is inaccurate and should be treated as an optional enhancement. In principle they would replace Inter's default character forms with alternate glyphs that sharpen the geometric precision of the letterforms. Even the monospace choice — Berkeley Mono over the expected SF Mono — signals an aesthetic preference for crafted tools.
...
| Token | Value | Role |
|---|---|---|
| `--canvas` | `#f4f5f8` | Page canvas — Linear Mercury White, light counterpart to Surface/Base |
| `--frame` | `#fafafa` | Primary frame container |
| `--card` | `#ffffff` | Cards, lifted content |
| `--view` | `#f7f7f8` | Active view container |
| `--chat` | `#f2f2f3` | Chat/input boxes |
| `--divider` | `#e4e4e7` | Navigation dividers, structural separators |

#### Light Text

| Token | Value | Role |
|---|---|---|
| `--ink` | `#222326` | Headlines, primary content — Nordic Gray, light counterpart to Text/Primary |
| `--ink-2` | `#4b4d54` | Secondary text, descriptions |
| `--ink-3` | `#8a8a8d` | Buttons, metadata, auxiliary text |

#### Light Accent & Border

| Token | Value | Role |
|---|---|---|
| `--accent` | `#5e6ad2` | Links, CTAs — identical to dark theme (brand color is theme-invariant) |
| `--accent-mention` | `#4f5bd0` | @mention indigo |
| `--accent-mentionbg` | `#eceefb` | Mention chip background |
| `--accent-strong` | `#4f5bd0` | Light-theme emphasis text — deeper than `#aab3ff` to reach WCAG AA |
| `--line` | `rgba(0, 0, 0, 0.08)` | Primary border — moonlight-on-glass inverted to black opacity |
| `--line-soft` | `rgba(0, 0, 0, 0.04)` | Soft separator |
| `--line-input` | `rgba(0, 0, 0, 0.02)` | Near-invisible input container |
| `--soft` | `rgba(0, 0, 0, 0.05)` | Subtle fill |
| `--soft-2` | `rgba(0, 0, 0, 0.02)` | Faintest fill |
| `--line-strong` | `rgba(0, 0, 0, 0.15)` | Strong separator |

Status colors (`--status-red` / `violet` / `indigo` / `green` / `cyan`) are identical in both themes — see the dark Status & Label Colors table above.
...
**OpenType Features**: `cv01` (character variant 01) and `ss03` (stylistic set 03) ⚠️ Measured: Linear's current rendering has `fontFeatureSettings = "normal"` — **not enabled**. The original description below should be read as a design-intent recommendation, not as Linear's actual behavior: the original text says they are enabled globally across all non-code text. In Inter, `cv01` provides alternate glyph forms for improved geometric consistency, while `ss03` adjusts specific characters for better readability in UI contexts. These features are deliberately omitted from code and input elements, where standard character recognition matters more than aesthetic refinement.
...
- **Enable `cv01` and `ss03` OpenType features** on all UI text — these character variants are integral to Linear's typographic identity. Omit them only for code and input elements. ⚠️ Measured: Linear's current rendering does not enable them (`fontFeatureSettings = "normal"`); this item is an optional enhancement recommendation, not something needed to match Linear's actual behavior.
...
**Prompt 5 — Code Block**
> Render a code block using Berkeley Mono at 12.25px, weight 400, line-height 1.3, letter-spacing -0.182px, color `rgb(208, 214, 224)`. Background is `rgba(255, 255, 255, 0.05)` with a `rgba(255, 255, 255, 0.08)` border and 2px border-radius. For larger code blocks (pre), increase to 14px with 24px line-height. Do NOT enable OpenType features cv01/ss03 on code elements — these are reserved for the Inter Variable UI face only. ⚠️ Measured: Linear's current UI renders `fontFeatureSettings = "normal"` (cv01/ss03 not enabled), so the "do not enable" rule for code coincides with the UI's actual behavior; the conclusion still holds but its premise has changed.

---

*Generated by Sparkbites — extracted from live CSS analysis*
*Light theme section (§2 Light Theme) added by CGC project — U3 dual-theme decision; values ported from web/app/globals.css.*
