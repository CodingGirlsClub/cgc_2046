...
```bash
# 1. 克隆本仓库（或下载 omp-access-pack/ 目录）
git clone https://github.com/CodingGirlsClub/cgc_2046.git
cd cgc_2046

# 2. 安装接入包
bash omp-access-pack/install.sh install
...
| 卸载 | `bash omp-access-pack/install.sh remove`（只删本包文件与条目，保留备份） |
...
```bash
bash omp-access-pack/install.sh install --url http://localhost:4000/mcp
```
...
本接入包验证时的 OMP 版本：____（实现时回填）。升级 OMP 后请重跑 `omp-access-pack/docs/verify-checklist.md`。
