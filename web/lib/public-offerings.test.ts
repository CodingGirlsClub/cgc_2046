...
describe("fetchPublicOfferings（E-5 #50 公开发现页数据层）", () => {
  it("event 走 PUBLIC_LIST_EVENTS，返回 results；空 → []", async () => {
    queryMock.mockResolvedValue({
      data: { listEvents: { results: eventRows } },
    });
...
describe("submitEnrollment（E-5 #50 createEnrollment mutation 契约）", () => {
  it("提交 input 并返回 result", async () => {
    mutateMock.mockResolvedValue({
      data: {
        createEnrollment: {
          result: { id: "enr1", status: "confirmed" },
          errors: [],
        },
      },
    });
...
describe("parseSponsorshipTiers（E-3 #48 JsonString 数组解析）", () => {
  ...
  it("结构非法项静默丢弃；null/非数组 → []", () => {
    expect(
      parseSponsorshipTiers(["not-json", JSON.stringify({ id: "x" })]),
    ).toEqual([]);
...
describe("serializeSponsorshipTier（保存路径 snake_case 契约钉测）", () => {
  it("产出后端白名单的 snake_case 键（amount_suggestion 等）", () => {
    ...
    expect(Object.keys(serialized).sort()).toEqual(
      ["amount_suggestion", "benefits", "exclusive", "id", "name"].sort(),
    );
    expect(serialized.amount_suggestion).toBe(10000);
    expect(serialized).not.toHaveProperty("amountSuggestion");
  });

  it("serialize → parse 往返恒等（保存后回显不丢字段）", () => {
    const tier = {
      id: "t2",
      name: "标准",
    ...
    expect(parseSponsorshipTiers([serializeSponsorshipTier(tier)])).toEqual([
      tier,
    ]);
  });
});
