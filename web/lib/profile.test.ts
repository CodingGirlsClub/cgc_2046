...
/**
 * 个人资料数据源测试（#69/#P1，唯一真实路径）。
 *
 * mock 双轨已删除（#1）：fetchCurrentProfile / updateCurrentProfile /
 * fetchProfileRoleSummary / Portfolio CRUD 均走真实 GraphQL。此处
...
describe("lib/profile Portfolio CRUD 真实分支（P1 myPortfolio 契约）", () => {
	...
	it("createPortfolioItem：errors 返回 → 抛错", async () => {
		...
		await expect(createPortfolioItem({ title: "" })).rejects.toThrow(
			"title 不能为空",
		);
