...
import { fetchPublicInitiative, type PublicInitiative } from "@/lib/graphql/initiatives";
...
export default function InitiativePage({ params }: { params: Promise<{ slug: string }> }) {
	...
	return <main className="public-catalog-main"><div className="public-catalog-container initiative-page">
		...
		{data.cities.map((group) => <section key={group.city} className="initiative-city"><h2>{group.city}</h2><ul className="public-catalog-grid">{group.events.map((event) => <li key={event.id}><Link href={`/events/${event.slug}`} className="public-catalog-card"><span className="public-catalog-card__head"><span className="public-catalog-card__title">{event.title}</span><strong>{event.status === "cancelled" ? t("cancelled") : event.qualificationStatus === "confirmed" ? t("qualified") : event.minParticipants ? t("shortBy", { count: Math.max(event.minParticipants - event.confirmedCount, 0) }) : t("open")}</strong></span><span>{event.startsAt ? new Date(event.startsAt).toLocaleString() : t("timeTbd")}</span></Link></li>)}</ul></section>)}
	</div></main>;
}
