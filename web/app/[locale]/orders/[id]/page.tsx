...
export default function OrderDetailPage() {
  ...
  // 轮询拉取：闭包捕获请求时 orderId，响应迟到时订单已被换渠道替换则丢弃
  // （旧版同款乱序窗口——换渠道后在飞的旧单响应覆盖新单，顺手同口径修复）
  const fetchStatus = useCallback(async () => {
    ...
    if (fresh && fresh.id === orderId) setOrder(fresh);
  }, [orderId]);
  ...
  useEffect(() => {
    discardOrderCredential(orderId);
  }, [orderId]);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    QRCode.toDataURL(dispatch.url, { width: 220, margin: 1 })
      .then((url) => {
  ...
  async function switchProvider(next: PaymentProvider) {
    ...
    try {
      ...
      if (payload?.result) {
        ...
        poll.reset();
        // 换渠道产生新单：同步 URL 到新单 id——orderId 是轮询查询/凭据读取的
        // 单源，不跟随则轮询固化查旧单（cancelled）直到超窗（advisor R1 基线缺陷）
        if (payload.result.id !== orderId) {
          router.replace(`/orders/${payload.result.id}`, { scroll: false });
        }
      } else {
