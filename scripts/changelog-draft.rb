...
#   2. 保留清单：feat/fix/perf/security 等用户可见变更（噪音已过滤）
#   3. 门禁提示：本批是否带 migrations / lockfile / 人工合并范围文件，mix.lock 变更时回 deps-image
#   4. Unreleased 现状：当前 [Unreleased] 段是否为空
...
commits = git("log", "--no-merges", "--format=%s", "#{m_hash}^1..origin/develop").lines.map(&:chomp)
...
changed = git("diff", "--name-only", "#{m_hash}^1..origin/develop").lines.map(&:chomp)
...
puts
puts "# 保留清单（据此写 [Unreleased] 草稿，同类浓缩、每条≤一行）"
kept.each { |c| puts "- #{c}" }
puts
puts "# 门禁提示"
