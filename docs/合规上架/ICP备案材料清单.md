...
- [ ] 微信订阅消息模板 ID ×19（approval_result / approval_reminder / event_reminder / enrollment_submitted / enrollment_completed / enrollment_check_in_code / speaker_accepted / speaker_completed / learning_stagnation / payment_succeeded / payment_received / payment_expired / refund_succeeded / refund_failed / event_qualification_confirmed / event_qualification_underfilled / event_schedule_changed / event_moderator_assigned / flashback_action_scheduled）——真源 = `.github/workflows/deploy.yml` 的 fail-closed 名单（+ kamal `backend/config/deploy.yml` env.secret、`backend/.env.example`，三处同集合），与 `backend/config/runtime.exs` 的 19 个 `WECHAT_MP_TEMPLATE_*` 键逐一对应；缺任一 → deploy 第一分钟红（非 boot 崩），守卫测试 `backend/test/cgc_2046/notifications/template_allowlist_test.exs`
