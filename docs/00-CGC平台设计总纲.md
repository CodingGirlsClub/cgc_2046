...
| WorkspaceMembership / MembershipRole / Role | 成员关系 + N:M 差异标签；成员资格 = membership 行 | 身份/租户 | 差异标签 Owner/Admin/Tutor/Volunteer/Learner；**成员中间态：request 策略申请中 → pending（JoinRequest）；invite_only 已受邀 → invited（Invitation）**；**角色语义（ADR-0006 / plan 017）：`member` 已退役，无标签 membership 即普通成员；「自定义角色」仍为未来能力（#71）** |
