...
6b. **schema 分文件——经 #844 重开(2026-09-24,招募域试点)**:第 6 条清偿后 26 天,graphql_schema.ex 从 2,771 行回升到 6,744 行,flashback 顶层字段从 0 增至 52 个(21 query + 31 mutation);近 3 个月(2026-06-24 起)171 次提交触达该文件,其中 102 次增删了声明行(@desc/field/object/input_object/arg)。方案:每个域一个 Absinthe notation 模块(web 层 `graphql_schema/<domain>.ex`),字段声明与 resolver 原样搬迁;某个域超过 500 行时,按内聚边界拆成子模块,域内私有的 helper 放 `graphql_schema/<domain>/helpers.ex`;跨域 helper 放 `Cgc2046Web.GraphqlSchema.Helpers`;最终目标:schema 和各域模块每个文件都在 500 行以内。本 PR 先做招募试点,其余域每个域单独一个 PR。
...
7b. **StatusTransition 声明式化——经 #846 升级(2026-09-24)**:根原语长出 action 级入口 `Cgc2046.StatusTransition.Change`(根部 `status_transition/change.ex`),Event/Course 六个 action(launch/close/cancel × 2)里逐字复制的约 40 行匿名 before_action CAS 块收敛为一行声明 `change({StatusTransition.Change, from: :draft, to: :open})`;错误文案逐字节不变(web 正则与 MCP 测试逐字依赖)。**刻意不纳入**(Change moduledoc 同文钉住,防后续评审再议):Initiative `transition/3` 与 Event/Course `:delete` 是行锁族(`SELECT … FOR UPDATE`、`from` 为列表、带 ready 门控、统一一句话报错),与条件 UPDATE 的 CAS 是两种机制(course.ex `:delete` 注释已论证);不因外形相似并进同一接口。StatusTransition 仍属「审批机制族」根部横切原语,不归任一 context。
