...
6b. **schema 分文件——经 #844 重开(2026-09-24,招募域试点)**:第 6 条清偿后 26 天,graphql_schema.ex 从 2,771 行回升到 6,744 行,flashback 顶层字段从 0 增至 52 个(21 query + 31 mutation);近 3 个月(2026-06-24 起)171 次提交触达该文件,其中 102 次增删了声明行(@desc/field/object/input_object/arg)。方案:按域拆 Absinthe notation 模块(web 层 `graphql_schema/<domain>.ex`),字段声明与 resolver 原样搬迁,跨域 helper 收进 `Cgc2046Web.GraphqlSchema.Helpers`;本 PR 先做招募试点,其余域每个域单独一个 PR。
