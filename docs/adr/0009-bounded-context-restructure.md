...
> 日期：2026-08-28 ｜ 状态：**已接受（Accepted）** ｜ 决策者：用户（product owner）
...
6. **D6：Payments 为 supporting domain，Order 锚 enrollment_id 不变。** 报名是唯一收费场景（赞助 v1 不收款），**放弃 `(subject_kind, subject_id)` 泛化**（无需求抽象）；Admission 独立后 order.ex 的 event/course 双分叉 JOIN 收敛为单 JOIN。资金对账（PaymentReconciliationWorker / ReplaySettlement）留 Payments 域内；Reconciliation Finding 底座与 /admin 对账页保持「底座共享 + 扫描器归各域」结构不动。
...
8. **D8：迁移策略——模型先行、四步 PR 序列**（数据库可重置，无绞杀/兼容层）：
   ...
   - **PR④ Payments 收敛 + 名额账本**：Order 引用收敛单 JOIN；名额账本表 + capacity 投影同步 + confirmed_count 展示投影化。
...
- **文档落点**：本 ADR + `领域模型定稿.md` §5.4 + CONTEXT.md 术语同步（Admission/Curriculum/供给物词条）先行；代码随四步序列跟进。
