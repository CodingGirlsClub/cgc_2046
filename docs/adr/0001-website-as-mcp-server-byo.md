...
> 日期：2026-08-01
> 决策来源：`docs/03-决策记录/grill-决策记录-2026-08-01.md`（D1–D14，grill-with-docs 交互式决策，14 条全量敲定）
> 关联文档：`CONTEXT.md`（术语表）｜ `docs/01-定稿设计/领域模型定稿.md`（领域模型）｜ `docs/01-定稿设计/用户旅程与Web功能清单.md`（页面清单）｜ `docs/01-定稿设计/技术调研与实施计划.md`（原技术方案）
...
- 领域模型修订：`docs/01-定稿设计/领域模型定稿.md`（Agent/AgentRun 重定义、MCP 工具、确认流、scope）——由领域建模工程师跟进。
- 页面清单修订：`docs/01-定稿设计/用户旅程与Web功能清单.md`（删对话/执行页，加连接引导页、审计查看页）。
- 实施计划修订：`docs/01-定稿设计/技术调研与实施计划.md` M2 大改（删 ash_ai，加 MCP server/连接器扩展）。
