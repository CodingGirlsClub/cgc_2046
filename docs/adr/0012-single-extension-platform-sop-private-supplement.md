...
生产构建读取私有 main，将唯一 tutor.md 校验后复制进 release，删除 staging。API 版本追加原始文件 SHA-256 前八位，镜像版本为 `<public SHA>-pb<hash8>`。私有仓库变更按人工约定走 PR、自查或 review 后合并，随后从公开 main 手动部署。不为私有分支强制保护升级套餐；接受维护者可绕过人工流程的风险，公开 main 与 production environment 保护不变。操作与验收见运维文档。
