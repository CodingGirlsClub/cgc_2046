...
> 本节为 §5.1「资源清单」context 列的目标态细化：取代「业务」单一 context 与 D-A4「Enrollment 归活动 context」。代码迁移分四步 PR 序列进行（ADR-0009 D8），迁移完成前代码目录以现状为准。
...
- **v1.5 同步（2026-08-28，ADR-0009）**：新增 §5.4 限界上下文地图——Events/Courses 分家、Admission（报名）独立（取代 D-A4）、Curriculum（教研）命名取代 Research、Sponsorship 独立、Offering 转正为发布语言读端口、Payments 确立 supporting domain 地位。迁移分四步 PR 序列；迁移完成前代码目录以现状为准。
