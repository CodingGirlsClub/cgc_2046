...
### 5.4 限界上下文地图（2026-08-28，ADR-0009）

> 本节为 §5.1「资源清单」context 列的细化：取代「业务」单一 context 与 D-A4「Enrollment 归活动 context」。代码迁移已按 ADR-0009 更正后的五步 PR 序列全部完成（2026-08-28：PR④ Sponsorship 独立为实施期补记——原 D8 序列漏排；PR⑤ 含 Payments 收敛 + 名额账本 + 展示投影回路 + Workflows/Learning/Reconciliation 归位、`Cgc2046.Api` 退役），代码目录与本表一一对应（2026-08-29 ADR-0010 A5 补三行后重核；例外 = 根部横切共享写原语 ApprovalClaim/ApprovalDeadline/StatusTransition 与读模型 PendingApprovals，刻意不归任一 context、根部驻留，见 CONTEXT.md 词条）。

| 限界上下文 | 类型 | 聚合/资源 | 关键边界规则 |
| --- | --- | --- | --- |
| Identity/Tenancy | supporting | User、Workspace、WorkspaceMembership、Role、Profile、Invitation、JoinRequest | 全域上游（OHS）；租户/身份判定唯一出处 |
| Events | core | Event、Venue、SpeakerInvitation | 活动运营面：场地、嘉宾；报名/缴费不内嵌 |
| Courses | core | Course | 课程供给面：排期/定价/报名策略/发布状态；内容本体在 Curriculum，本 context 持发布投影 |
| Offering | shared kernel | 供给物读投影（offering.ex）+ 共享纯函数内核（offering/：PriceTier / ScheduleValidation / EnrollmentBadge / Readiness） | Events↔Courses 共有的发布语言读契约（ADR-0009 D5）；纯读零写、无状态无表；**改动需 Events/Courses 两侧同时回归**（ADR-0010 G1-⑧ 正名） |
| Admission | core | Enrollment、InviteBatch、名额账本 | 报名生命周期唯一归属（Event/Course 语义同一，不复制）；名额占位/释放 CAS 在账本内完成；offering `confirmed_count` 为展示投影（信号最终一致） |
| Payments | supporting | Order、Provider（ACL→微信/支付宝） | 资金事实源；Order 锚 `enrollment_id` 单一引用（不泛化 subject_kind）；资金对账 worker 域内 |
| Curriculum（教研） | core | 教研产出物（outline/materials/issues/archive） | 产出物起草/审核/归档唯一归属；被 Events/Courses 实例化引用；被 Learning 读契约消费 |
| Sponsorship | core | Sponsorship、SponsorshipDelivery、SponsorshipTier | 两级赞助（Event 级/Workspace 级）+ 履约账本；Event 引用为软引用 |
| Learning | core | LearningRecord | 记忆挂人不挂报名；消费 Curriculum 已发布内容 |
| Workflows | generic | WorkflowDefinition、WorkflowRun、Step、SignalLog | 确定性编排引擎；依赖方向恒为 workflow → 业务 Action 接口 |
| Notification | supporting | Fanout/Service/Templates | 收件人解析 + 入队唯一归属 |
| Miniprogram | channel supporting | ShareScheme（分享链接缓存） | 小程序渠道接入面；方案 A（ADR-0010 ⑩，2026-08-29）后仅剩 ShareScheme——邀请渠道码归 Accounts（`invitation_codes`）、订阅授权余额归 Notifications（`notification_consents`） |
| Platform Reporting | supporting | Reconciliation Finding 底座 | 「底座共享 + 扫描器归各域」：Finding 存储/刷新语义/admin 页共享，链路扫描与资金对账扫描各归其域 |
| Integrations | ACL/infra | Wechat Client/Requester、SendCloud 适配器等 | 外部服务唯一翻译层（anti-corruption layer）；无领域逻辑，被各 context 调用 |
| MCP gateway | interface layer | 工具面/鉴权/审计 | 适配器层，不含领域逻辑 |

**跨 context 映射关系**：Events/Courses → Admission = OHS + Published Language（Offering 供给物读契约：status/capacity/deadline/price_tiers）；Admission → Payments = Customer/Supplier；Payments → 支付渠道 = ACL；Curriculum → Courses/Learning = OHS（读契约）；各域 → Notification/信号总线 = 事件驱动（信号名为发布语言，不改名）。**跨 context 写点清零**：每个 context 独占自己数据的更新权（D2 名额账本为最后消除的一点）。
...
- **v1.5 同步（2026-08-28，ADR-0009）**：新增 §5.4 限界上下文地图——Events/Courses 分家、Admission（报名）独立（取代 D-A4）、Curriculum（教研）命名取代 Research、Sponsorship 独立、Offering 转正为发布语言读端口、Payments 确立 supporting domain 地位。迁移序列见 ADR-0009 D8。
- **v1.6 同步（2026-08-28，ADR-0009 实施完成）**：更正后的五步 PR 序列全部落地（Sponsorship 独立单列为 PR④，原 D8 漏排）；`Cgc2046.Api` domain 退役删除，Workflows / Learning / Reconciliation 归位各自 domain；§5.4 去除「待代码迁移」标注，代码目录与 §5.4 地图一一对应。
