...
> 本节为 §5.1「资源清单」context 列的细化：取代「业务」单一 context 与 D-A4「Enrollment 归活动 context」。代码迁移已按 ADR-0009 更正后的五步 PR 序列全部完成（2026-08-28：PR④ Sponsorship 独立为实施期补记——原 D8 序列漏排；PR⑤ 含 Payments 收敛 + 名额账本 + 展示投影回路 + Workflows/Learning/Reconciliation 归位、`Cgc2046.Api` 退役），代码目录与本表一一对应。
