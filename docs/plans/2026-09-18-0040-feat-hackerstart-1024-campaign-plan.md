...
- R2. 页面按定稿信息架构组织：Hero（三 CTA + 十周年刻度条「2016 · 成立 —— 第 10 年 —— 2026 · 2¹⁰＝1,024 场」）→ 为什么是现在 → 我要参加（15/120/30 分钟节奏卡 + 班型事实行 + CTA）→ 参与者 FAQ → 成为志愿者专区（三职位小卡 + 批次卡〔只写「首批招募进行中」，不写死日期——真实状态由申请页承载〕+ 支持一行；深读内容在申请页）→ 时间线（三节点）→ 我们是谁（十年数字带 + 证据链接墙 + 杠杆句）→ 赞助合作 Partnership（lead 一句话讲合作边界〔多品牌同批 / 课程统一研发 / 零抽成〕+ 五项收益阶梯〔品牌同行 512-2,048 本轮口径 / 数据与案例 / 渠道网络 / ESG 与品牌信任 / 全程曝光〕+ 交付能力三列 + 可以一起做的事 + 合作结构胶囊 + CTA）→ 留存位（公众号关注）→ Footer（含志愿者回链）。
- R3. 三入口落点：「我要参加」→ `/initiatives`；「成为志愿者」→ 志愿者申请页（R10）；「品牌合作」→ `mailto:partners@codingirlsclub.com`。
- R4. 口径纪律：不出现任何厂商名（含 OpenClacky）；不出现商务价格（参与者押金是报名必要信息，另按押金口径告知）；历史累计（2016-2025）与本轮计划（2026.10-2027）分开标注、永不混用；关键数字沿 2 的幂标注（2⁰ 启动日 / 2³ / 2⁵ / 2⁶ 首期 64 场 / 2⁷ / 2¹⁰），幂标记全站统一样式（等宽 + 玫红 + 缩小），与主数字视觉分离。押金金额（87 元）不参与幂叙事、不解释定价由来。
...
- F1. 参与者路径
  ...
  - **Actors:** A1。
  - **Steps:** 选「我要参加」→ 阅读节奏卡与 FAQ → CTA 跳 `/initiatives` → 按城市查看场次 → 既有报名流（87 元押金）。
...
- F3. 品牌路径
  ...
  - **Actors:** A4。
  - **Steps:** 阅读赞助合作 Partnership 段（一批 32 场滚动开批；五项收益 + 合作边界）→ mailto partners@ → 48 小时内人工回复。
