...
## 一、现状证据（2026-08-13, develop c987260）
...
| Event/Course 死枚举 + 无 end 动作 | `event.ex:30` `@status_values [:draft,:open,:closed,:cancelled]`；仅 `launch` 动作（`:180-188`）；无 close/cancel 动作；无 `end_at` 字段；全库无 `event.ended`/`course.ended` 生产者 |
...
| web 现状 | `web/app/w/[slug]` 仅 workspace 页 + 占位卡；无 `/events/[slug]`、`/courses/[slug]`；`web/app/admin/*` 已建成（2026-08-10 admin plan 已实施） |
| slice E 骨架 | #46-51 全 OPEN（label ready-for-agent）；#39 CLOSED COMPLETED；新增 #122（E-7）/ #123（E-8）/ #124（E-9）/ #125（E-10） |
...
### Idea 3 · 公开发现面 → E-5 #50 增强

**设计**
1. **读策略翻转**：Event/Course `status=open` ⇒ 匿名可读；非 open 维持成员专属；写入不动。公开字段白名单 = 决策 D2。
2. **公开宿主页**：`/events/[slug]`、`/courses/[slug]` + 公开发现页列表，消费 #40 已建已测查询；报名轻量表单消费 `create_enrollment` mutation（已暴露）；赞助/邀请入口挂宿主页（随 E-3/E-4 补上）。
...
**验收**：匿名 GET open event 200 且字段在白名单内；非 open 匿名拒绝；表单提交 → Enrollment 落库；GO/NO-GO 缺项产生 warning + readiness 可查。
...
[✓ Phase 0] 判据 + 报名最小接线（PR #119 已合并）
      │
      ▼
[Phase 1] E-9 生命周期级联（#124）                          ← 纯后端地基；去重依赖 PR #121 合入
      │
      ├──────────────► [Phase 2] E-5 公开发现面（Idea 3）   ← 可与 Phase 1 并行
      │
      ▼
[Phase 3] E-8 审批控制台（#123，Idea 4）                    ← 报名审批 E2E；赞助审批前置
      │
      ▼
[Phase 4] 三线落地：E-3 赞助（#48，+Idea 6 账本）、E-4 邀请（#49）、E-2 订阅方收尾（#47）
      │
      ▼
[Phase 5] E-7 学习设计（#122，Idea 2）                      ← 设计可即刻并行；实现依赖 E-2
      │
      ▼
[Phase 6] E-10 对账扫描（#125，Idea 7）                     ← 规则随 workflow 累积，最后落地
      │
      ▼
[Phase 7] E-6 #51 端到端验证（按 AGENTS.md 确定性分层）
```

- **无依赖（可立即启动）**：Phase 1（E-9，去重基座依赖 PR #121 合入）、E-7 设计文档。
- **并行窗口**：Phase 1 ∥ Phase 2 ∥ E-7 设计 ∥ E-4 邀请（仅需 Phase 2 token 着陆页，核心独立）。
- **强依赖**：E-3 ← Phase 1（ended）+ Phase 3（审批面）+ D5（账本）；E-2 订阅方 ← Phase 1（幂等表）；E-10 ← 各 workflow 规则。

## 四、决策清单（8 项 — 2026-08-13 用户签核「全部同意」，全部按推荐批准）

| # | 决策 | 结论 | 性质 |
|---|---|---|---|
| D1 | 新增 E-7/E-8/E-9/E-10 issue + 修订 #46/#47/#48/#50 body | ✅ 已执行（#122-#125，#46/#47/#48/#50） | 工作登记 |
| D2 | 公开字段白名单（读策略翻转的安全语义） | ✅ 批准：标识/描述/时间/策略/状态公开；capacity/confirmed_count 不公开 | 安全变更 |
...
| D8 | 对账规则与消费纪律 | ✅ 批准：四条规则；报告落 /admin 对账页 | 设计定稿 |

## 五、范围边界

- **不做**：收款/支付（payment_pending→paid，二期）；候补；Event/Course 收敛单一 Offering（ideation 被裁 #21，收益待第三 workflow 落地后单独 brainstorm）；平台托管 Learner 档；邀请批量/候选池（二期）；makegood 闭环（依赖续期）；教研 #8 答疑交互（非 slice E）。
